...
byte note=0;//只有27~86能发出声音
byte resetMIDI=3;
byte ledPin=10;
int instrument=30;//Instruments在这里不重要
int scale=-24;//因为Range27~86
...
int key1,key2,key3,key4,key5,key6,key7,key8,key9,key10,key11,key12;
...
void Noteon(int *previous,int tune,int scale){
  ...
  {
    note=tune+scale;
    if(note>=27&&note<=86){noteOn(0,note,120);
    *previous=1;}
  }
}
void Noteoff(int *previous,int tune,int scale){
  ...
  {
    note=tune+scale;
    if(note>=27&&note<=86){noteOff(0,note,120);
    *previous=0;}
  }
}
//第二个八度
